	  ...
	  function($routeProvider, $translateProvider) {
	    $routeProvider.
			...
			$translateProvider.translations('en', {
				...
				BUY_DEVICE_IDENTITY_TYPE:'Identity Type',
				BUY_DEVICE_NRIC:'NRIC',
				BUY_DEVICE_PASSPORT:'Passport',
				BUY_DEVICE_OTHERS:'Others',
				BUY_DEVICE_UPLOAD_IMAGE:'Upload Image',
				BUY_DEVICE_SCAN:'Scan',
				BUY_DEVICE_COY_ROY:'COY ROC #',
				BUY_DEVICE_NO:'No',
				BUY_DEVICE_EMAIL:'Email',
				BUY_DEVICE_IMEI_NO:'IMEI No',
				BUY_DEVICE_OTHERS:'Employment / Work Permit / Student Pass / Spl Pass No',
				BUY_DEVICE_FULLNAME:'Full Name',
				BUY_DEVICE_ADDR_1:'Address Line 1',
				BUY_DEVICE_ADDR_2:'Address Line 2',
				BUY_DEVICE_ADDR_3:'Address Line 3',
				BUY_DEVICE_CONTACT:'Contact No',
				BUY_DEVICE_CITY:'City',
				BUY_DEVICE_PROVINCE:'Province/State',
				BUY_DEVICE_COUNTRY:'Country',
				BUY_DEVICE_ZIPCODE:'Zip Code',
				BUY_DEVICE_SUBMIT:'Submit',
				BUY_DEVICE_PRINT:'Print',
				BUY_DEVICE_CANCEL:'Cancel'								
...
sampleApp.controller('PersonalCtrl', function ($scope, $http, personalDetails, AuthService ) {
		...
		var originalCustomer;
	 	...
	 	if (personalDetails != null) {
	 		...
	 		if (AuthService.getCurrentRole() == 'Retailer') {
				...
				$scope.retailer.directorName = personalDetails.data.directorName;
				$scope.retailer.mobileNumber = personalDetails.data.mobileNumber; 
				$scope.retailer.retailerId = personalDetails.data.retailerId;				
				$scope.retailer.coyContactNumber = personalDetails.data.coyContactNumber;	
				originalRetailer = angular.copy($scope.retailer); 		
	 		}
	 		if (AuthService.getCurrentRole() == 'User') {
				...
				originalCustomer = angular.copy($scope.customer); 		
	 		}	 					
	 	}
	 	...
	 	$scope.doRetailerUpdateCancel = function() {
			$scope.retailer = angular.copy(originalRetailer);
	 	}
	 	
	 	$scope.doCustomerUpdateCancel = function() {
		...
		$scope.doRetailerUpdate = function() {
				var dataObj = {
					...
					fullAddress: $scope.retailer.fullAddress,
					email: $scope.retailer.email,
					directorName: $scope.retailer.directorName,  	
					coyContactNumber: $scope.retailer.coyContactNumber,						
					retailerId: $scope.retailer.retailerId
				};				
                $http({
                        method : 'POST',
                        url : PERSONALINFOUPDATE_RETAILER,
                        data: dataObj                        
                }).success(function(data, status, headers, config) {
                	if (data.STATUS=='SUCCESS') {
                		$scope.retailerupdate.status = 'SUCCESS';
                		$scope.retailerupdate.msg = data.MESSAGE;
                		originalRetailer = angular.copy($scope.retailer);
                	} else {
                		$scope.retailerupdate.status = 'FAILED';
                		$scope.retailerupdate.msg = data.MESSAGE;                	
                	}
		...
		$scope.doPartnerUpdate = function() {
                ...
                $http({
                        method : 'POST',
                        url : PERSONALINFOUPDATE_PARTNER,
                        data: dataObj
                }).success(function(data, status, headers, config) {
                	console.log(data);
                	if (data.STATUS=='SUCCESS') {
		...
		$scope.doCustomerUpdate = function() {
                ...
                $http({
                        method : 'POST',
                        url : PERSONALINFOUPDATE_CUSTOMER,
                }).success(function(data, status, headers, config) {
                	console.log(data);
                	if (data.STATUS=='SUCCESS') {
                		$scope.customerupdate.status = 'SUCCESS';
                		$scope.customerupdate.msg = data.MESSAGE;
                		originalCustomer = angular.copy($scope.customer);
                	} else {
                		$scope.customerupdate.status = 'FAILED';
                		$scope.customerupdate.msg = data.MESSAGE;                	
                	}
                }).error(function(data, status, headers, config) {
                	$scope.customerupdate.status = 'FAILED';      
...
sampleApp.controller('TopupHistoryCtrl', function ($scope, $http,  $location, logincheck) {
	...
	$scope.doEnquire = function() {
		...
		var res = $http({
			method: "POST",
			url: TOPUPHISTORY_URL+'?fromDate='+startDateTime+'&toDate='+endDateTime
		});
